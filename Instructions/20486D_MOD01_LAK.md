...
2. In the **Get Started** section of the welcome screen, click **Create a new project**.

3. In the **Create a new project** dialog box, in the navigation pane, expand the first dropdown (**All languages**), and then filter to **Visual C#**.

4. Expand the third dropdown (**All project types**) and select **Web**.

5. In the filtered pane, click **ASP.NET Core Web App** and then **Next**

6. In the **Project Name** box, type **ActorsRazorPages**.

7. Replace the content of the **Location** box with **[Repository Root]\Allfiles\Mod01\Labfiles\01_ActorsRazorPages_begin**, and then click **OK**.

8. In the result pane of the **New ASP.NET Core Web App - ActorsRazorPages** dialog box, select **ASP.NET 5.0 (Current)** from the dropdown at the top of the dialog box.

9. Ensure that the **Configure for HTTPS** check box is not selected.

10. Leave every other answer untouched: ensure that **Authentication Type** is set to **None**.  **Enable Docker** and  **Enable Razor runtime configuration** are disabled. 

11. Click **Create**.

12. In the **ActorsRazorPages –  Microsoft Visual Studio** window, on the **Debug** menu, click **Start Without Debugging**.

13. In the address bar of the browser, note the port number that appears at the end of the URL **http://localhost:[port]**. You will use the port number during this lab.

14. In the browser, in the navigation bar, click **Privacy** to review its content.
...
2. In the **Add New Item – ActorsRazorPages** dialog box, click **Razor Page - Empty**.
...
20. In the **ActorsRazorPages - Microsoft Visual Studio** window, in **Solution Explorer**, right-click **wwwroot**, point to **Add**, point to **New Folder**, name the new folder **images**.

21 right-click **images**, point to **Add**, and then click **Existing Item**.

22. In the dialog box, navigate to **[Repository Root]\Allfiles\Mod01\Labfiles\01_ActorsRazorPages_begin\Images**, select all the images, and then click **Add**.

23. In the **ActorsRazorPages - Microsoft Visual Studio** window, in **Solution Explorer**,  right-click **Pages**, point to **Add**, and then select **New folder**.

24. In the **NewFolder** box, type **Actors**, and then press Enter.
...
25. In the **Add New Item - ActorsRazorPages** dialog box, click **Web**, and then, in the result pane, click **Razor Page - Empty**.
...
38. In the **_Layout.cshtml** code window, locate the following code:
  ```cs
  <li class="nav-item">
    <a class="nav-link text-dark" asp-page="/TestPage">Test Page</a>
  </li>
```
...
39. Place the cursor after the located code, press Enter, and then type the following code:
  ```cs
  <li class="nav-item">
    <a class="nav-link text-dark" asp-page="/Actors/Index">Actors</a>  
  </li>
```
...
3. In the browser, in the navigation bar, click **Test Page**  to view its content.
...
7. In the browser, click **Close**.

8. In the **ActorsRazorPages - Microsoft Visual Studio** window, on the **File** menu, click **Exit**.

>**Results**: At the end of this exercise, you were able to build a simple Razor Pages application in Visual Studio.
...
5. In the filtered pane, click **ASP.NET Core Web API** and then **Next**

5.  In the **Project Name** box, type **CakeStoreApi**.
...
7. In the result pane of the **New ASP.NET Core Web Application - CakeStoreApi**  dialog box, ensure that the *Target Framework*  is **.NET 5.0** and the *Configure for HTTPS* check box is **NOT** selected.

8. In the **New ASP.NET Core Web Application - CakeStoreApi**  dialog box, enable the **Enable OpenAPI support**  and then click **Create**.
...
1. In the **CakeStoreApi - Microsoft Visual Studio** window, in **Solution Explorer**, expand **Controllers**, and then click  **WeatherForecastController.cs**.
    ...
    >**Note**: The **Get()** method returns an array of 5 forecast objects (consisting of a *Date*, a *temperatureC*, a *temperatureF*, and a *Summary*). These 5 objects are randomly generated.

2. In the **CakeStoreApi – Microsoft Visual Studio** window, on the **Debug** menu, click **Start Without Debugging**. 

    >**Note**: The browser displays an Open API interface..

3. In the browser, click on the **Blue GET button**.

4. Click on the **Try it out** button. Then on the **Blue Execute Button**.  Look at the results

5. You can click on th execute button a few more times to see the result.
...
3. In the browser, in the address bar, type **http://localhost:[port]/api/CakeStore**, and then press Enter.

    >**Note**: The browser displays a list of cakes in JSON format.
    
4. In the browser, in the address bar, type **http://localhost:[port]/api/CakeStore/1**, and then press Enter.
...
9. In the browser, in the navigation bar, click **Contact** to review its content.

10. In the browser, click **Close**.
...
3. In the browser, in the navigation bar, click **Test Page** to review its content.
